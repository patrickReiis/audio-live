...
const io = new Server(server, {
  ...
  maxHttpBufferSize: 5e6, // Increase buffer size to 5MB for audio chunks
});
...
io.on('connection', (socket) => {
  ...
  socket.on('audio-chunk', (audioChunk, sessionId) => {
    if (sessionId && activeSessions.has(sessionId)) {
      ...
      console.log(`Received audio chunk: ${audioChunk.length || 'unknown'} bytes for session ${sessionId}`);
